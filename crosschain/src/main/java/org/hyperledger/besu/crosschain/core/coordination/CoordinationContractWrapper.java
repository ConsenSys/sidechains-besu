...
public class CoordinationContractWrapper {
  ...
  public CoordinationContractWrapper(final SECP256K1.KeyPair creds) {
    this.credentials =
        Credentials.create(creds.getPrivateKey().getD().toString(16));
  }
  ...
  public boolean start(
    ...
    RawTransactionManager tm = new RawTransactionManager(web3j, this.credentials, blockchainId.longValue(), RETRY, COORDINATION_BLOCK_PERIOD_IN_MS);
    CrosschainCoordinationV1 contractWrapper = CrosschainCoordinationV1.load(contractAddress.getHexString(), web3j, tm, this.freeGasProvider);
    ...
    BigInteger hashOfMessage = new BigInteger(1, message.getCrosschainTransactionHash().getByteArray());
    ...
    LOG.info("Crosschain Coordination Contract start message: signature: {}", message.getSignature().getHexString());
    RemoteFunctionCall<TransactionReceipt> tx = contractWrapper.start(
    ...
    try {
      ...
      LOG.error("Exception thrown while submitting start message transaction to Crosschain Coordination Contract: {}", ex.toString());
      return false;
    }
    LOG.info("Crosschain Coordination Contract start message transaction receipt: {}", receipt.toString());
    return true;
  }
}
